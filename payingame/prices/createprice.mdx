...
Multiple prices can be given to the product(s). So you will be able to fill in prices for EUR, USD and any token that you want to accept payments on (for Web3 items).
You can as well give temporary discounts based on this.

<img
  src="/images/showprice.png"
  alt="addproject"
  height="100"
  className="rounded-lg"
/>

Above gives an example on how to create different prices for your product. The way the prices are shown is from top down. So if a price has a discount between a certain time period, its best to move them on top so that the system will take it into consideration.
